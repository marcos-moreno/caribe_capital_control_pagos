...
class LoginView(act: LoginActivity) : BaseView<LoginActivity>(act) {
    ...
    fun onLoginResult(data: String, advisor: Advisor = Advisor(), local: Boolean) {
        ...
        if (userName.isEmpty()) {
            ...
            activity.runOnUiThread {
                ...
                var intent = Intent(activity, com.emprendamos.mx.emprendamosfin.GroupsActivity::class.java)
    ...
    fun setVersionCode() {
        activity.text_info.text = "Caribe Capital v.${BuildConfig.VERSION_NAME}"
    }
}